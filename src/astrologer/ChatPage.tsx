...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`https://listee-backend.onrender.com:5000/api/messages/${CURRENT_USER_ID}/${ASTRO_ID}`)
      .then(res => { if (res.data.messages) setMessages(res.data.messages); });
    axios.get("https://listee-backend.onrender.com:5000/api/questions").then(res => {
  ...
  const sendMessage = async (customText?: string) => {
    ...
    try {
      ...
      if (selectedFile && !customText) {
        ...
        const res = await axios.post("https://listee-backend.onrender.com:5000/api/messages", { text: textToSend, senderId: CURRENT_USER_ID!, receiverId: ASTRO_ID!, isBot: false });
        savedUserMsg = res.data.message;
      }
      ...
      if (timeLeft > 0 && questionIndex < botQuestions.length) {
        ...
        setTimeout(async () => {
          const botRes = await axios.post("https://listee-backend.onrender.com:5000/api/messages", { text: botQuestions[questionIndex].answer, senderId: ASTRO_ID!, receiverId: CURRENT_USER_ID!, isBot: true });
          setIsTyping(false);
