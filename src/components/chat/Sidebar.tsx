...
export default function Sidebar({ onSelect, selectedUser }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await axios.get("https://aqua-goat-506711.hostingersite.com/api/doctors");
            ...
            .map(async (u) => {
              try {
                const msgRes = await axios.get(
                  `https://aqua-goat-506711.hostingersite.com/api/messages/last/${CURRENT_USER_ID}/${u._id}`
                );
