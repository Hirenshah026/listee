...
const ChatPage = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`aqua-goat-506711.hostingersite.com/api/messages/${CURRENT_USER_ID}/${ASTRO_ID}`)
      .then(res => { if (res.data.messages) setMessages(res.data.messages); });
    axios.get("aqua-goat-506711.hostingersite.com/api/questions").then(res => {
  ...
  const sendMessage = async (customText?: string) => {
    ...
    try {
      ...
      if (selectedFile && !customText) {
        ...
        const res = await axios.post("aqua-goat-506711.hostingersite.com/api/messages/", formData);
        savedUserMsg = res.data.message;
      } else {
        const res = await axios.post("aqua-goat-506711.hostingersite.com/api/messages", { text: textToSend, senderId: CURRENT_USER_ID!, receiverId: ASTRO_ID!, isBot: false });
        savedUserMsg = res.data.message;
      }
      ...
      if (timeLeft > 0 && questionIndex < botQuestions.length) {
        ...
        setTimeout(async () => {
          const botRes = await axios.post("aqua-goat-506711.hostingersite.com/api/messages", { text: botQuestions[questionIndex].answer, senderId: ASTRO_ID!, receiverId: CURRENT_USER_ID!, isBot: true });
          setIsTyping(false);
