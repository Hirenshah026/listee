      ...
      <div className="w-full max-w-[450px] relative flex flex-col">
        ...
        <div className="absolute top-6 left-4 right-4 flex justify-between items-center z-50">
          <div className="bg-black/30 backdrop-blur-md px-3 py-1 rounded-full border border-white/10 flex items-center gap-2">
            <span className="text-white text-xs font-bold flex items-center gap-1 uppercase tracking-tighter"><Users size={12} className="text-yellow-400"/> {viewers}</span>
          </div>
        ...
        <div className="flex-1 relative bg-zinc-900 flex items-center justify-center">
          ...
          {/* Chat - Shifted up to avoid input overlap */}
          <div className="absolute bottom-[90px] left-0 w-full px-4 max-h-[35%] overflow-y-auto flex flex-col items-start gap-2 scrollbar-hide">
            {messages.map((m, i) => (
              <div key={i} className="bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-2xl border border-white/5 shadow-lg">
          ...
          {status === "Live" && isMuted && (
            <button onClick={() => {setIsMuted(false); remoteVideoRef.current?.play();}} className="absolute inset-0 m-auto w-16 h-16 bg-yellow-500 rounded-full flex items-center justify-center z-50 shadow-2xl transition-transform active:scale-90">
        ...
        <div className="p-4 bg-zinc-950 border-t border-white/5 flex gap-2 z-[100]">
          <form onSubmit={handleSendMessage} className="flex-1 flex gap-2">
            <input value={chatInput} onChange={e => setChatInput(e.target.value)} className="flex-1 bg-zinc-900 text-white rounded-full px-5 py-2.5 text-sm outline-none border border-white/10 focus:border-yellow-500" placeholder="Ask something..." />
            <button type="submit" className="bg-yellow-500 p-2.5 rounded-full text-black active:scale-90 transition-all"><Send size={18}/></button>
          </form>
