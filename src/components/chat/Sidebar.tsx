...
export default function Sidebar({ onSelect, selectedUser }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await axios.get("https://listee-backend.onrender.com:5000/api/doctors");
            ...
            .map(async (u) => {
              try {
                const msgRes = await axios.get(
                  `https://listee-backend.onrender.com:5000/api/messages/last/${CURRENT_USER_ID}/${u._id}`
                );
