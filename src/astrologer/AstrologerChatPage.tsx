...
const AstrologerChatPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://192.168.105.180:5000/api/messages/users/${ASTRO_ID}`)
  ...
  const openChat = async (u: ChatUser, index: number) => {
    ...
    const res = await axios.get(
      `http://192.168.105.180:5000/api/messages/${ASTRO_ID}/${u._id}`
    );
  ...
  const sendMessage = async () => {
    ...
    const res = await axios.post("http://192.168.105.180:5000/api/messages", {
      senderId: ASTRO_ID,
