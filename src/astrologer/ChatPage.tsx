...
import CallOverlay from "./CallOverlay"; // CallOverlay import kiya
...
const ChatPage = () => {
  ...
  const callOverlayRef = useRef<any>(null); // Call ref
  ...
  useEffect(() => {
    ...
    axios.get(`http://10.184.233.180:5000/api/messages/${CURRENT_USER_ID}/${ASTRO_ID}`)
      .then(res => { if (res.data.messages) setMessages(res.data.messages); });
    axios.get("http://10.184.233.180:5000/api/questions").then(res => {
  ...
  const sendMessage = async (customText?: string) => {
    ...
    try {
      ...
      if (selectedFile && !customText) {
        ...
        const res = await axios.post("http://10.184.233.180:5000/api/messages/", formData);
        savedUserMsg = res.data.message;
      } else {
        const res = await axios.post("http://10.184.233.180:5000/api/messages", { text: textToSend, senderId: CURRENT_USER_ID!, receiverId: ASTRO_ID!, isBot: false });
        savedUserMsg = res.data.message;
      }
      ...
      if (timeLeft > 0 && questionIndex < botQuestions.length) {
        ...
        setTimeout(async () => {
          const botRes = await axios.post("http://10.184.233.180:5000/api/messages", { text: botQuestions[questionIndex].answer, senderId: ASTRO_ID!, receiverId: CURRENT_USER_ID!, isBot: true });
          setIsTyping(false);
    ...
    <div className="fixed inset-0 w-full overflow-hidden bg-[#efeae2] flex justify-center">
      ...
      <div ref={containerRef} className="w-full max-w-[450px] flex flex-col relative overflow-hidden bg-[#efeae2]" style={{ height: '100dvh' }}>
        ...
        <header className="absolute top-0 w-full h-[60px] bg-yellow-400 flex items-center justify-between px-3 shadow-md z-[200]">
          ...
          {/* New Call Buttons in Header */}
          <div className="flex gap-4 mr-2">
            <button onClick={() => callOverlayRef.current?.startCall('voice')} className="p-2 active:scale-90 text-xl">📞</button>
            <button onClick={() => callOverlayRef.current?.startCall('video')} className="p-2 active:scale-90 text-xl">📹</button>
          </div>
        </header>
        ...
        {showProfileModal && (
          <div className="absolute inset-0 bg-black/90 z-[300] flex flex-col items-center justify-center" onClick={() => setShowProfileModal(false)}>
            <img src={astrologer?.image || "/banners/astrouser.jpg"} className="max-w-[90%] max-h-[70vh] object-contain shadow-2xl" alt="astro profile" />
          </div>
        )}
