  ...
  return (
    <div className="fixed inset-0 w-full overflow-hidden bg-[#efeae2] flex justify-center">
      ...
      <div ref={containerRef} className="w-full max-w-[450px] flex flex-col relative bg-[#efeae2]" style={{ height: '100dvh' }}>
        ...
        <header className="absolute top-0 w-full h-[65px] bg-white flex items-center justify-between px-3 shadow-sm border-b z-[200]">
          <div className="flex items-center gap-2">
            <button onClick={() => navigate(-1)} className="text-2xl font-bold text-gray-800 p-1">←</button>
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => setShowProfileModal(true)}>
              <div className="relative">
                <img src={astrologer?.image || "/banners/astrouser.jpg"} className="w-11 h-11 rounded-full border border-gray-100 object-cover" alt="astro" />
                <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></div>
              </div>
              <div>
                <p className="font-bold text-[15px] text-gray-800 leading-tight">{astrologer?.name}</p>
                <p className="text-[10px] font-bold text-gray-500 uppercase flex items-center gap-1">
                  <span className="text-orange-500">⏱️</span> {Math.floor(timeLeft/60000)}:{(Math.floor(timeLeft/1000)%60).toString().padStart(2,'0')}
                </p>
              </div>
            </div>
          </div>
          <div className="flex gap-3">
            <button onClick={() => callOverlayRef.current?.startCall('voice')} className="p-2 text-xl hover:bg-gray-50 rounded-full transition-colors">📞</button>
            <button onClick={() => callOverlayRef.current?.startCall('video')} className="p-2 text-xl hover:bg-gray-50 rounded-full transition-colors">📹</button>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto px-4 pt-[75px] pb-4 bg-[url('https://user-images.githubusercontent.com/15075759/28719144-86dc0f70-73b1-11e7-911d-60d70fcded21.png')] bg-fixed">
            ...
            return (
              <div key={i}>
                ...
                <div className={`flex ${isMe ? "justify-end" : "justify-start"} mb-2`}>
                  <div className={`max-w-[85%] p-1.5 rounded-xl shadow-sm relative ${isMe ? "bg-[#dcf8c6] rounded-tr-none" : "bg-white rounded-tl-none border-l-4 border-orange-400"}`}>
          ...
          {isTyping && <div className="text-[12px] text-gray-500 italic bg-white inline-block px-3 py-1 rounded-lg ml-2 animate-pulse shadow-sm">Astro is typing...</div>}
          <div ref={bottomRef} />
        </main>
        ...
        <footer className="flex-none p-3 pb-6 bg-[#f0f2f5] flex items-center gap-2 border-t z-[100]">
          <div className="flex-1 bg-white rounded-3xl flex items-center px-3 py-1.5 border border-gray-200 shadow-sm">
            <button onClick={() => fileInputRef.current?.click()} className="text-xl text-gray-500 mr-2 rotate-45 hover:text-gray-700">📎</button>
            <input 
              ref={inputRef} 
              type="text" 
              value={input} 
              ...
              className="flex-1 bg-transparent border-none outline-none py-1.5 text-gray-800" 
          ...
          <button 
            ...
            onClick={() => sendMessage()} 
            className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg transition-transform active:scale-90 ${canChat ? 'bg-[#00a884]' : 'bg-gray-400'}`}
          >
            ➤
          </button>
        </footer>
        ...
        {showProfileModal && (
          <div className="fixed inset-0 bg-black/95 z-[500] flex flex-col items-center justify-center p-4" onClick={() => setShowProfileModal(false)}>
            <img src={astrologer?.image || "/banners/astrouser.jpg"} className="max-w-full max-h-[70vh] object-contain rounded-lg shadow-2xl" alt="astro profile" />
            <p className="text-white mt-5 font-bold text-xl">{astrologer?.name}</p>
            <button className="mt-8 text-white bg-white/10 px-6 py-2 rounded-full">Close</button>
          </div>
        )}

        {imagePreview && (
          <div className="fixed inset-0 bg-black z-[600] flex flex-col">
            <header className="p-4 text-white flex justify-between items-center bg-black/40">
              <button onClick={() => { setImagePreview(null); setSelectedFile(null); }} className="text-2xl">✕</button>
              <span className="font-bold uppercase tracking-wider text-sm">Send Photo</span>
              <div></div>
            </header>
            <div className="flex-1 flex items-center justify-center p-2">
              <img src={imagePreview} className="max-w-full max-h-[70vh] object-contain" alt="preview" />
            </div>
            <div className="p-4 flex gap-2 bg-black/60 backdrop-blur-md">
              <input 
                type="text" 
                value={input} 
                onChange={e => setInput(e.target.value)} 
              ...
              />
              <button onClick={() => sendMessage()} className="bg-[#00a884] w-14 h-14 rounded-full text-white flex items-center justify-center shadow-lg active:scale-90">✔</button>
            </div>
          </div>
        )}
      </div>
