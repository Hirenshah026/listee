...
const AstrologerChatPage = () => {
  ...
  const fetchUsersData = async () => {
    if (!ASTRO_ID) return;
    try {
      const res = await axios.get(`http://10.184.233.180:5000/api/messages/users/${ASTRO_ID}`);
      const userList = res.data.users || [];
      const enriched = await Promise.all(userList.map(async (u: ChatUser, i: number) => {
        const lastRes = await axios.get(`http://10.184.233.180:5000/api/messages/last/${ASTRO_ID}/${u._id}`);
        return { 
            ...u, 
        ...
        {!selectedUser ? (
          <>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {users.map(u => (
                <div key={u._id} onClick={() => { setSelectedUser(u); axios.get(`http://10.184.233.180:5000/api/messages/${ASTRO_ID}/${u._id}`).then(res => setMessages(res.data.messages || [])); }} className="flex items-center gap-3 p-3 border-b cursor-pointer hover:bg-gray-50 transition-colors">
