...
const AstroLiveHost = () => {
  ...
  useEffect(() => {
    ...
    // Host ke liye message receive logic
    ...
    socket.on("answer-from-viewer", async ({ from, answer }) => {
      if (pcs.current[from]) await pcs.current[from].setRemoteDescription(new RTCSessionDescription(answer));
    });

    socket.on("ice-candidate", ({ candidate, from }) => {
      if (pcs.current[from]) pcs.current[from].addIceCandidate(new RTCIceCandidate(candidate)).catch(() => {});
    });
    ...
    <div className="fixed inset-0 bg-black overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col p-4 pointer-events-none">
        ...
        <div className="pointer-events-auto">
          <button onClick={isLive ? () => window.location.reload() : startLive} className={`w-full py-4 rounded-full font-bold shadow-2xl ${isLive ? 'bg-red-600 text-white' : 'bg-yellow-500 text-black'}`}>
