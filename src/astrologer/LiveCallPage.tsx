...
const LiveCallPage = () => {
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    const msgData = { roomId: ROOM_ID, user: "User", text: chatInput, id: `m-${Date.now()}` };
      ...
      <div className="w-full max-w-[450px] relative bg-zinc-900 flex flex-col h-full">
        <div className="absolute top-0 w-full p-4 z-50 flex justify-between items-center bg-gradient-to-b from-black to-transparent">
          <div className="flex items-center gap-2">
            ...
            <div className="text-white text-xs font-bold">Astro Live <br/> <span className="flex items-center gap-1 text-[10px] text-zinc-300 font-normal"><Users size={10}/> {viewers} Viewers</span></div>
          </div>
        ...
        <div ref={chatContainerRef} className="absolute bottom-24 w-full px-4 max-h-[160px] overflow-y-auto flex flex-col gap-1.5 scrollbar-hide">
          {messages.map((m, i) => (
            <div key={i} className="bg-black/40 backdrop-blur-sm text-white text-xs p-2 rounded-lg border border-white/10 self-start">
        ...
        <div className="p-4 flex gap-2 bg-black border-t border-white/5">
          <form onSubmit={handleSendMessage} className="flex-1 flex gap-2">
            <input value={chatInput} onChange={e => setChatInput(e.target.value)} className="flex-1 bg-white/10 rounded-full px-4 text-white text-xs py-2 outline-none border border-white/10" placeholder="Chat..." />
            <button className="bg-yellow-500 p-2 rounded-full text-black"><Send size={16}/></button>
          </form>
